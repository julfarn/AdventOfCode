...
class PiecewiseDiagonalFunction:
    ...
    # returns local minima of the within the interval [a,b]
    ...
    # returns suspected local minima of the within the interval [a,b]
    def sus_local_minima(self, a, b):
        minima = [a]
        for i in range(len(self.steps)):
            if(a <= self.step_start(i) < b): # minimum of step
                minima.append(self.step_start(i))
            if(a <= self.step_end(i) < b): # minimum of next step/gap
                minima.append(self.step_end(i))

        return minima

    # returns suspected local minima 
    def all_local_minima(self):
        minima = [0]
        for i in range(len(self.steps)):
            minima.append(self.step_start(i))
            minima.append(self.step_end(i))
    ...
    # returns pre-image of interval [a,b] as a list with entries [pre_a, height, width] (i.e. a step)
    def preimage(self, a,b): 
        ret = []
        laststepend = 0
        for i in range(len(self.steps)):
            # check image of region before step
            if(laststepend<b and self.step_start(i) > a): # any intersection
                pre_a = laststepend + max(a - laststepend,0)
                pre_b = self.step_start(i) - max(self.step_start(i) -b ,0)
                if(pre_b - pre_a > 0):
                    ret.append([pre_a,0,pre_b-pre_a])

            # check image of step
            if(self.step_start_height(i)<b and self.step_end_height(i) > a): # any intersection
                pre_a = self.step_start(i) + max(a - self.step_start_height(i),0)
                pre_b = self.step_end(i) - max(self.step_end_height(i) -b ,0)

                if(pre_b - pre_a > 0):
                    ret.append([pre_a,self.step_height(i),pre_b-pre_a])
            laststepend = self.step_end(i)

        # check image of region after all steps
        if(laststepend<b): # any intersection
            pre_a = laststepend + max(a - laststepend,0)
            pre_b = pre_a + (b-laststepend)
            if(pre_b - pre_a > 0):
                ret.append([pre_a,0,pre_b-pre_a])

        return ret
    
    def preimage_steps(self, a): # returns ONLY STEP pre-image of half-open interval [a, infinity] as a list with entries [pre_a, height, width] (i.e. a step)
        ret = []
        laststepend = 0
        for i in range(len(self.steps)):
            # check image of step
            if(self.step_end_height(i) > a): # any intersection
                pre_a = self.step_start(i) + max(a - self.step_start_height(i),0)
                pre_b = self.step_end(i)
...
comp = compose("seed", "location")
loc_minima = []
for i in range(int(len(seeds)/2)):
    #loc_minima += comp.local_minima(seeds[2*i], seeds[2*i]+seeds[2*i+1])
    loc_minima += comp.sus_local_minima(seeds[2*i], seeds[2*i]+seeds[2*i+1])
    #loc_minima += comp.all_local_minima()

locations = [comp.ev(x) for x in loc_minima]
print(min(locations))
...
# replace letter S with appropriate segment symbol
for seg in link_symbols:
    if seg_dir[0] in links[seg]  and seg_dir[1] in links[seg]:
        maze[start_pos_r] = maze[start_pos_r][:start_pos_c] + seg + maze[start_pos_r][start_pos_c+1:]

# Follow the maze!
pipe_coords = [[]] * len(maze) 
for i in range(len(maze)):
    pipe_coords[i] = []
pipe_coords[start_pos_r].append(start_pos_c)
steps = 0
while(steps == 0  or seg_c[0] != seg_c[1] or seg_r[0] != seg_r[1]):
    for path in [0,1]:
        ...
        #      ".")
        pipe_coords[seg_r[path]].append(seg_c[path])
...
for row in range(len(maze)):
    ...
    lastleftcorner = "F"
    for col in range(len(maze[row])):
        if(col in pipe_coords[row]): # update state
            if maze[row][col] in "FL":
                lastleftcorner = maze[row][col]
            if maze[row][col] in "|":
                inside = not inside
            if maze[row][col] == "J" and lastleftcorner == "F":
                inside = not inside
            if maze[row][col] == "7" and lastleftcorner == "L":
                inside = not inside

        elif(inside): # add cell if on inside
            inner_count+= 1
